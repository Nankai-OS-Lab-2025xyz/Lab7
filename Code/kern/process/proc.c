...
load_icode(unsigned char *binary, size_t size)
{
    ...
    // 设置 SPP = 0 (User Mode)  返回用户态
    // 设置 SPIE = 1 (Enable Interrupts，允许中断) 返回后开启中断
    // SPP=0: 返回用户态; SPIE=1: 返回后开中断
    tf->status = (sstatus | SSTATUS_SPIE) & ~SSTATUS_SPP;

    // 设置 sepc 为 ELF 程序的入口地址，sret指令返回时跳转到此地址
    tf->epc = elf->e_entry;

    // 设置 sp 为用户栈顶地址，程序运行时的栈指针
    tf->gpr.sp = USTACKTOP;
    ret = 0;
